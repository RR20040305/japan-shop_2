...
### CRUD запросы PostgreSQL (19)
# 1. Создать пользователя (POST)
fetch('/api/simple-users', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ first_name: 'Иван', last_name: 'Петров', age: 25 })
}).then(r => r.json()).then(console.log)
# 2. Получить всех пользователей (GET)
fetch('/api/simple-users').then(r => r.json()).then(console.log)
# 3. Получить одного пользователя (GET /:id)
fetch('/api/simple-users/1').then(r => r.json()).then(console.log)
# 4. Обновить возраст (PATCH)
fetch('/api/simple-users/1', {
  method: 'PATCH',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ age: 30 })
}).then(r => r.json()).then(console.log)
# 5. Удалить пользователя (DELETE)
fetch('/api/simple-users/1', { method: 'DELETE' }).then(r => r.json()).then(console.log)

### MongoDB запросы CRUD (20)
# 1. Создать пользователя (POST)
fetch('/api/mongo-users', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ first_name: 'Полина', last_name: 'Алмазова', age: 28 })
}).then(r => r.json()).then(console.log);
# 2. Получить список всех пользователей (GET)
fetch('/api/mongo-users').then(r => r.json()).then(console.log);
# 3. Получить одного пользователя по ID (GET)
fetch('/api/mongo-users/665c7f... ').then(r => r.json()).then(console.log);
# 4. Обновить пользователя (PATCH)
fetch('/api/mongo-users/665c7f...', {
  method: 'PATCH',
  headers: { 'Content-Type': 'application/json' },
  body: JSON.stringify({ age: 30 })
}).then(r => r.json()).then(console.log);
# 5. Удалить пользователя (DELETE)
fetch('/api/mongo-users/665c7f...', { method: 'DELETE' }).then(r => r.json()).then(console.log);
...
# Выполните два раза подряд GET‑запрос. Первый пойдёт в базу, второй должен взять из кэша.
fetch('/api/simple-users').then(r => r.json()).then(console.log)
#
setTimeout(() => fetch('/api/simple-users').then(r => r.json()).then(console.log), 2000)
# Открыть логи. 
docker-compose logs -f backend1
# При втором запросе в логах появится строка:
⚡ Cache hit: /api/simple-users

### Проверка nginx (HAProxy) (22)
curl --no-keepalive http://localhost:8080/server
curl --no-keepalive http://localhost:8080/server
curl --no-keepalive http://localhost:8080/server

### Контейнеризация (Docker) (23)
# Убедиться, что все контейнеры запущены
docker-compose ps
# Посмотреть логи конкретного сервиса
docker-compose logs backend1
docker-compose logs nginx
# Проверить, что данные PostgreSQL сохраняются в томе
docker volume inspect japan-shop_4_pgdata



## 📌 Практическая работа №19: Работа с PostgreSQL (реляционная СУБД)

### Задача  
Создать API для управления пользователями с хранением данных в PostgreSQL. Реализовать CRUD-операции через Sequelize ORM (Object‑Relational Mapping (ORM) библиотека для Node.js. Она позволяет работать с реляционными базами данных (PostgreSQL, MySQL, MariaDB, SQLite, Microsoft SQL Server) через объекты JavaScript, вместо того чтобы писать SQL‑запросы вручную.).

### Выполненные работы

1. **Установка и настройка PostgreSQL** – локально или через Docker.  
2. **Создание модели `User`** с полями: `id`, `first_name`, `last_name`, `age`, `created_at`, `updated_at`.  
3. **Реализация эндпоинтов**:
   - `POST /api/sql/users` – создание пользователя.
   - `GET /api/sql/users` – получение всех пользователей.
   - `GET /api/sql/users/:id` – получение одного пользователя.
   - `PATCH /api/sql/users/:id` – обновление.
   - `DELETE /api/sql/users/:id` – удаление.
4. **Подключение к проекту «Токийский дрифт»** – пользователи магазина хранятся в PostgreSQL (таблица `Users`).

### Результат  
Все данные пользователей магазина надёжно хранятся в PostgreSQL. Реализована полная CRUD‑операции через API, поддерживаются роли (`user`, `seller`, `admin`) и блокировка.

---

## 📌 Практическая работа №20: Работа с MongoDB (NoSQL)

### Задача  
Реализовать альтернативное API для управления пользователями с использованием MongoDB и Mongoose. Выполнить CRUD-операции.

### Выполненные работы

1. **Установка MongoDB** (локально или через Docker). MongoDB – это документо-ориентированная NoSQL база данных. В отличие от реляционных баз (как PostgreSQL, MySQL), она не использует таблицы, строки и столбцы. Вместо этого данные хранятся в виде документов (формат BSON – бинарный JSON), объединённых в коллекции. Mongoose – это Object Data Modeling (ODM) библиотека для Node.js, которая предоставляет удобный интерфейс для работы с MongoDB.  
2. **Создание схемы `User`** с полями: `first_name`, `last_name`, `age`, `created_at`, `updated_at`.  
3. **Разработка эндпоинтов** (аналогично практической №19, но с MongoDB).  
4. **Интеграция в проект** – для демонстрации работы с NoSQL создан отдельный маршрут `/api/nosql/users` (в рамках практики 19 был SQL, в 20 – NoSQL). В проекте «Токийский дрифт» используется PostgreSQL для пользователей, но для выполнения задания отдельно реализован сервис на MongoDB.

### Результат  
Разработано отдельное API на MongoDB, демонстрирующее гибкость документо‑ориентированной модели. В проекте сохранены оба подхода (SQL и NoSQL) для разных сущностей.
...
### Задача  
Добавить кэширование ответов для часто запрашиваемых маршрутов (`/api/users`, `/api/users/:id`, `/api/products`, `/api/products/:id`) с использованием Redis для ускорения работы и снижения нагрузки на БД.

### Выполненные работы

1. **Установка и настройка Redis** (Docker-контейнер).  Redis (от Remote Dictionary Server) – это высокопроизводительное in‑memory хранилище данных типа «ключ‑значение». Оно работает в оперативной памяти (RAM), что обеспечивает микросекундные задержки при чтении/записи, но данные могут быть потеряны при перезапуске, если не настроено сохранение на диск.
2. **Создание middleware `cacheMiddleware`** для проверки наличия данных в кэше. cacheMiddleware – это специальная промежуточная функция (middleware) в Express, которая перехватывает входящие запросы до того, как они достигнут основного обработчика. Её задача – проверить, есть ли уже закэшированный ответ для этого запроса (обычно в Redis). Если есть – ответ возвращается мгновенно, минуя базу данных и всю дальнейшую логику. Если нет – запрос передаётся дальше, а после получения данных (из БД) они сохраняются в кэш для будущих вызовов.  
3. **Создание функции `saveToCache`** для сохранения ответа в Redis с TTL.  
4. **Интеграция в нужные маршруты** – добавление `cacheMiddleware` перед основным обработчиком.  
5. **Очистка кэша** – при изменении данных (создание, обновление, удаление пользователя/товара) соответствующие ключи удаляются.

### Результат  
При первом запросе данные загружаются из БД и сохраняются в Redis на заданное время (1 минута для пользователей, 10 минут для товаров). Повторные запросы в течение TTL мгновенно возвращаются из кэша, что значительно ускоряет ответ и снижает нагрузку на базу данных. При изменении данных кэш инвалидируется.
...
### Задача  
Настроить балансировку HTTP‑трафика между несколькими экземплярами бэкенда с помощью Nginx и HAProxy. Обеспечить отказоустойчивость (health checks) и распределение запросов.

### Выполненные работы

1. **Создание трёх идентичных backend-серверов** на Node.js (Express), каждый возвращает свой `SERVER_ID`.  
2. **Настройка Nginx** в роли балансировщика:  
   - Определён `upstream backend` с тремя серверами (`backend1`, `backend2`, `backend3`).  
   - Использован алгоритм `round-robin` (по умолчанию).  
   - Добавлены параметры `max_fails=2 fail_timeout=30s` для проверки доступности.  
   - Резервный сервер (`backup`) указан для демонстрации.  
3. **Настройка HAProxy** как альтернативного балансировщика (на порту 81).  
4. **Тестирование** – с помощью `curl` убедились, что запросы распределяются по очереди между серверами.

Nginx — это, прежде всего, высокопроизводительный веб-сервер и обратный прокси-сервер. Его основное предназначение — принимать и обрабатывать запросы от клиентов (браузеров) по протоколам HTTP и HTTPS, а затем возвращать им ответ (например, веб-страницу или данные от API).

### Результат  
В проекте реализована полная балансировка нагрузки с помощью Nginx (основной балансировщик) и HAProxy (демонстрационный). Система устойчива к сбоям одного из узлов: недоступные серверы автоматически исключаются, и трафик распределяется между работающими экземплярами.
...
### Задача  
Упаковать приложение (бэкенд, базу данных, балансировщик) в Docker-контейнеры и управлять стеком через Docker Compose. Настроить сети, тома и переменные окружения.

### Выполненные работы

1. **Установка WSL и Docker** (согласно инструкции).  
2. **Создание `Dockerfile`** для backend-сервиса (Node.js).  
3. **Написание `docker-compose.yml`**, включающего:
   - PostgreSQL (контейнер с официальным образом).
   - Три экземпляра backend (сборка из `Dockerfile`).
   - Nginx (балансировщик) с пробросом портов 80 и 443.
   - Общую сеть `app-net` для взаимодействия.
   - Тома для сохранения данных PostgreSQL.
4. **Конфигурирование переменных окружения** (`.env`) для базы данных, JWT, VAPID.  
5. **Запуск стека** командой `docker compose up --build`.  
6. **Тестирование** – проверка балансировки, сохранения данных после перезапуска, работы статических файлов.

WSL – это подсистема Windows для запуска полноценного Linux-окружения (например, Ubuntu) прямо внутри Windows, без необходимости устанавливать виртуальную машину или настраивать двойную загрузку.
Docker – это платформа для контейнеризации приложений. Она позволяет упаковать приложение вместе со всеми его зависимостями (библиотеки, Node.js, PostgreSQL, конфиги) в изолированный контейнер, который можно запустить на любой системе, где установлен Docker.

### Результат  
Всё приложение полностью контейнеризировано. Одна команда `docker compose up` разворачивает PostgreSQL, три бэкенда и nginx. Контейнеры изолированы, данные сохраняются через тома, а параметры конфигурации передаются через `.env`. Это обеспечивает воспроизводимость окружения на любой платформе, поддерживающей Docker.
